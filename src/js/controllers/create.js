...
angular.module('copayApp.controllers').controller('createController',
  function ($scope, $rootScope, $location, $timeout, $log, lodash, go, profileService, configService, isCordova, gettext, isMobile, derivationPathHelper, correspondentListService) {
    ...
    const updateRCSelect = function (n) {
      ...
      if ($scope.requiredCosigners > n || !$scope.requiredCosigners) { $scope.requiredCosigners = parseInt(n / 2 + 1); }
    };

    const updateSeedSourceSelect = function (n) {
      self.seedOptions = [{
        id: 'new',
    ...
    for (let i = 0; i < $scope.totalCosigners - 1; i++) { this.cosigners.push({}); }
    ...
    this.setTotalCosigners = function (tc) {
      ...
      const newLen = tc - 1;
      if (newLen > oldLen) {
        for (let i = oldLen; i < newLen; i++) { self.cosigners.push({}); }
      } else if (newLen < oldLen) { self.cosigners.length = newLen; }
    ...
    this.onCorrespondentSelected = function (device_address) {
      ...
      if (device_address === 'new') { go.path('correspondentDevices.addCorrespondentDevice'); }
    };
    ...
    this.create = function (form) {
      ...
      if (self.cosigners.length !== $scope.totalCosigners - 1) { return setError('invalid number of cosigners'); }
