  ...
  function ($scope, $timeout, $log, configService, profileService, storageService, go, isCordova) {
    ...
    function saveDeviceName() {
      ...
      configService.set(opts, (err) => {
        if (err) { self.$emit('Local/DeviceError', err); }
      });
    }

    configService.get((err, config) => {
      if (err) { throw Error('failed to read config'); }
    ...
    this.setWalletType = function () {
      ...
      fs.writeFile(userConfFile, JSON.stringify({ bLight }, null, '\t'), 'utf8', (err) => {
        if (err) { throw Error(`failed to write conf.json: ${err}`); }
        const conf = require('byteballcore/conf.js');
        if (!conf.bLight) { throw Error('Failed to switch to light, please restart the app'); }
        self.step = 'device_name';
        $scope.$apply();
      });
    };
    ...
    this.create = function (noWallet) {
      if (self.creatingProfile) { return console.log('already creating profile'); }
