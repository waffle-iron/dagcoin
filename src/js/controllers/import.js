...
angular.module('copayApp.controllers').controller('importController',
      ...
      async.series([
        ...
        function (callback) {
          ...
          if (existsConfJson) {
            ...
            const _conf = require(`${dbDirPath}conf.json`);
            _conf.bLight = false;
        ...
        function (callback) {
          fileSystemService.readdir(`${dbDirPath}temp/`, (err, fileNames) => {
            async.forEach(fileNames, (name, callback2) => {
              ...
              if (err) return callback(err);
              fileSystemService.nwRmDir(`${dbDirPath}temp/`, () => {
                callback();
              });
            });
          });
  ...
  function decrypt(buffer, password) {
    ...
    const decipher = crypto.createDecipheriv('aes-256-ctr', crypto.pbkdf2Sync(password, '', 100000, 32, 'sha512'), crypto.createHash('sha1').update(password).digest().slice(0, 16));
    const arrChunks = [];
  ...
  function unzipAndWriteFiles(data, password) {
    if (isCordova) {
      zip.loadAsync(decrypt(data, password)).then((zip) => {
        if (!zip.file('light')) {
          self.imported = false;
          ...
          writeDBAndFileStorageMobile(zip, (err) => {
  ...
  self.iosWalletImportFromFile = function (fileName) {
    $rootScope.$emit('Local/NeedsPassword', false, null, (err, password) => {
      if (password) {
        const backupDirPath = `${window.cordova.file.documentsDirectory}/Byteball/`;
        fileSystemService.readFile(backupDirPath + fileName, (err, data) => {
          if (err) return showError(err);
