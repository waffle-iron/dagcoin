...
angular.module('copayApp.controllers').controller('acceptCorrespondentInvitationController',
  function ($scope, $rootScope, $timeout, configService, profileService, isCordova, go, correspondentListService) {
    ...
    function handleCode(code) {
      ...
      if (!matches) { return setError('Invalid pairing code'); }
      const pubkey = matches[1];
      const hub = matches[2];
      const pairing_secret = matches[3];
      if (pubkey.length !== 44) { return setError('Invalid pubkey length'); }
      ...
      correspondentListService.acceptInvitation(hub, pubkey, pairing_secret, (err) => {
