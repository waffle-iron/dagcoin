  ...
  function ($rootScope, $scope, $timeout, profileService, go, gettext, confirmDialog, notification, $log, isCordova) {
    ...
    self.toggle = function () {
      ...
      if (!self.credentialsEncrypted) {
        if (!self.show) { $rootScope.$emit('Local/BackupDone'); }
        self.show = !self.show;
      }

      if (self.credentialsEncrypted) { self.passwordRequest(); }
      ...
      try {
        ...
        if (e.message && e.message.match(/encrypted/) && fc.isPrivKeyEncrypted()) {
          ...
          profileService.unlockFC(null, (err) => {
            ...
            if (!self.show && self.credentialsEncrypted) { self.show = !self.show; }
