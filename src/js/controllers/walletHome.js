...
angular.module('copayApp.controllers').controller('walletHomeController', function ($scope, $rootScope, $timeout, $filter, $modal, $log, notification, isCordova, profileService, lodash, configService, storageService, gettext, gettextCatalog, nodeWebkit, addressService, confirmDialog, animationService, addressbookService, correspondentListService, autoUpdatingWitnessesList) {
  ...
  const disablePaymentRequestListener = $rootScope.$on('paymentRequest', (event, address, amount, asset, recipient_device_address) => {
  ...
  $scope.openSharedAddressDefinitionModal = function (address) {
    ...
    const ModalInstanceCtrl = function ($scope, $modalInstance) {
      ...
      walletGeneral.readMyAddresses((arrMyAddresses) => {
        walletDefinedByAddresses.readSharedAddressDefinition(address, (arrDefinition, creation_ts) => {
    ...
    if (asset && indexScope.arrBalances.filter(balance => (balance.asset === asset)).length === 0) { return console.log(`i do not own anything of asset ${asset}`); }
  ...
  this.setAddress = function (forceNew) {
    ...
    // Address already set?
    if (!forceNew && self.addr[fc.credentials.walletId]) { return; }

    if (indexScope.shared_address && forceNew) { throw Error('attempt to generate for shared address'); }
    ...
    $timeout(() => {
      addressService.getAddress(fc.credentials.walletId, forceNew, (err, addr) => {
        ...
        if (err) {
          self.addrError = err;
        } else if (addr) { self.addr[fc.credentials.walletId] = addr; }
  ...
  this.openCustomizedAmountModal = function (addr) {
    ...
    const ModalInstanceCtrl = function ($scope, $modalInstance) {
      ...
      $scope.submitForm = function (form) {
        if ($scope.index.arrBalances.length === 0) { return console.log('openCustomizedAmountModal: no balances yet'); }
        const amount = form.amount.$modelValue;
        const asset = $scope.DAGCOIN_ASSET;
        if (!asset) { throw Error('no asset'); }
        const amountInSmallestUnits = (asset === 'base') ? parseInt((amount * $scope.unitValue).toFixed(0)) : (asset === constants.DAGCOIN_ASSET ? parseInt((amount * $scope.dagUnitValue).toFixed(0)) : amount);
        $timeout(() => {
      ...
      $scope.shareAddress = function (uri) {
        if (isCordova) {
          if (isMobile.Android() || isMobile.Windows()) { window.ignoreMobilePause = true; }
  ...
  this.submitForm = function () {
    if ($scope.index.arrBalances.length === 0) { return console.log('send payment: no balances yet'); }
    const fc = profileService.focusedClient;
    ...
    const form = $scope.sendForm;
    if (!form) { return console.log('form is gone'); }
    if (self.bSendAll) { form.amount.$setValidity('validAmount', true); }
    if (form.$invalid) {
    ...
    if (fc.isPrivKeyEncrypted()) {
      profileService.unlockFC(null, (err) => {
        if (err) { return self.setSendError(err.message); }
...
/*    var comment = form.comment.$modelValue;
    ...
    const recipient_device_address = assocDeviceAddressesByPaymentAddress[address];
    let amount = form.amount.$modelValue;
    let merkle_proof = '';
    if (form.merkle_proof && form.merkle_proof.$modelValue) { merkle_proof = form.merkle_proof.$modelValue.trim(); }
    amount *= dagUnitValue;
    amount = Math.round(amount);

    const current_payment_key = `${asset}${address}${amount}`;
    if (current_payment_key === self.current_payment_key) { return $rootScope.$emit('Local/ShowErrorAlert', 'This payment is already under way'); }
    ...
    $timeout(() => {
      profileService.requestTouchid((err) => {
        ...
        if (self.binding) {
          if (!recipient_device_address) { throw Error('recipient device address not known'); }
          const walletDefinedByAddresses = require('byteballcore/wallet_defined_by_addresses.js');
          ...
          walletDefinedByKeys.issueNextAddress(fc.credentials.walletId, 0, (addressInfo) => {
            ...
            if (self.binding.type === 'reverse_payment') {
              ...
              if (self.binding.feed_type === 'explicit') { arrEventCondition = arrExplicitEventCondition; } else if (self.binding.feed_type === 'merkle') { arrEventCondition = arrMerkleEventCondition; } else if (self.binding.feed_type === 'either') { arrEventCondition = ['or', [arrMerkleEventCondition, arrExplicitEventCondition]]; } else							{ throw Error(`unknown feed type: ${self.binding.feed_type}`); }
              var arrDefinition = ['or', [['and', [['address', address], arrEventCondition]], ['and', [['address', my_address], ['in data feed', [[configService.TIMESTAMPER_ADDRESS], 'timestamp', '>', Date.now() + Math.round(self.binding.timeout * 3600 * 1000)]]]]]];
              var assocSignersByPath = {
                'r.0.0': {
                  address,
                  member_signing_path: 'r',
          ...
          fc.sendMultiPayment(opts, (err) => {
            ...
            if (recipient_device_address) { // show payment in chat window
              ...
              if (binding && binding.reverseAmount) { // create a request for reverse payment
                if (!my_address) { throw Error('my address not known'); }
                const paymentRequestCode = `byteball:${my_address}?amount=${binding.reverseAmount}&asset=${encodeURIComponent(binding.reverseAsset)}`;
  ...
  this.canSendExternalPayment = function () {
    if ($scope.index.arrBalances.length === 0) // no balances yet, assume can send
      { return true; }
    if (!$scope.index.arrBalances[$scope.index.assetIndex].is_private) { return true; }
    const form = $scope.sendForm;
  ...
  this.openBindModal = function () {
    ...
    const ModalInstanceCtrl = function ($scope, $modalInstance) {
      ...
      $scope.arrPublicAssetInfos = indexScope.arrBalances.filter(b => !b.is_private).map((b) => {
        ...
        if (b.asset === 'base') { info.displayName = self.unitName; } else if (b.asset === constants.DAGCOIN_ASSET) { info.displayName = self.dagUnitName; } else					{ info.displayName = `of ${b.asset.substr(0, 4)}`; }
        return info;
      });
  ...
  function getAmountInSmallestUnits(amount, asset) {
    ...
    if (asset === 'base') { amount *= self.unitValue; } else if (asset === constants.DAGCOIN_ASSET) { amount *= self.dagUnitValue; }
  ...
  this.setForm = function (to, amount, comment, asset, recipient_device_address) {
    ...
    if (amount) {
      if (asset === 'base') { amount /= this.unitValue; }
      if (asset === constants.DAGCOIN_ASSET) { amount /= this.dagUnitValue; }
  ...
  this.resetForm = function () {
    ...
    if (form && form.amount) {
      form.amount.$pristine = true;
      form.amount.$setViewValue('');
      if (form.amount) { form.amount.$render(); }
  ...
  this.setSendAll = function () {
    ...
    if (indexScope.arrBalances[indexScope.assetIndex].asset === 'base') {
      ...
      let full_amount = indexScope.arrBalances[indexScope.assetIndex].stable;
      if (indexScope.arrBalances[indexScope.assetIndex].asset === constants.DAGCOIN_ASSET) {
        full_amount /= this.dagUnitValue;
      }
  ...
  this.onAddressChange = function (value) {
    ...
    if (value.indexOf(`${self.protocol}:`) === 0) { return this.setFromUri(value); }
    return value;
  };
  ...
  this.showCorrespondentListToReSendPrivPayloads = function (btx) {
    ...
    const ModalInstanceCtrl = function ($scope, $modalInstance, $timeout, go, notification) {
      ...
      $scope.sendPrivatePayments = function (correspondent) {
        const indivisible_asset = require('byteballcore/indivisible_asset');
        const wallet_general = require('byteballcore/wallet_general');
        indivisible_asset.restorePrivateChains(btx.asset, btx.unit, btx.addressTo, (arrRecipientChains, arrCosignerChains) => {
          wallet_general.sendPrivatePayments(correspondent.device_address, arrRecipientChains, true, null, () => {
            modalInstance.dismiss('cancel');
            go.history();
            $timeout(() => {
  ...
  this.sendAll = function (amount, feeStr) {
    const self = this;
    const msg = gettextCatalog.getString('{{fee}} will be deducted for bitcoin networking fees', {
      fee: feeStr,
    });

    confirmDialog.show(msg, (confirmed) => {
      if (confirmed) { self._doSendAll(amount); }
    });
  };
