...
if (no_key == false) { // Reminder: Any changes to the script below must also be made to the else clause and vice versa.
  ...
  https.get(`https://api.crowdin.com/api/project/${crowdin_identifier}/export?key=${crowdin_api_key}`, (res) => {
    ...
    res.on('data', (chunk) => {
      const resxml = chunk.toString('utf8');
      console.log(resxml);
      ...
      https.get(`https://crowdin.com/download/project/${crowdin_identifier}.zip`, (res) => {
        let data = [],
          dataLen = 0;
        ...
        res.on('data', (chunk) => {
          ...
          for (var i in files) {
            if (files[i] != 'template.pot') {
              let po_file = fs.readFileSync(path.join(__dirname, `po/${files[i]}`), 'utf8');
              const po_array = po_file.split('\n');
              for (const j in po_array) {
                if (po_array[j].slice(0, 5) == 'msgid') {
                  var source_text = po_array[j].slice(5);
                } else if (po_array[j].slice(0, 6) == 'msgstr') {
                  const translate_text = po_array[j].slice(6);
                  ...
                  if (source_text != translate_text) {
                    ...
                    const lang_pos = po_file.search('"Language: ') + 11;
                    let po_start = po_file.slice(0, lang_pos);
                    let po_locale = po_file.slice(lang_pos, lang_pos + 5);
                    let po_end = po_file.slice(lang_pos + 5);
